...
from utils_ventas import eliminar_producto, actualizar_carrito, agregar_producto, cancelar_compra, finalizar_compra, modificar_cantidad
...
def crear_pestaña_ventas(notebook, productos):
    ...
    ttk.Button(seleccionar_frame, text="Cancelar Compra", command=lambda: cancelar_compra(carrito, carrito_tree, total_label, productos)).pack(side="left", padx=5)

    # Tabla del carrito
    carrito_tree = ttk.Treeview(frame, columns=("ID", "Producto", "Cantidad", "Precio","Subtotal"), show="headings", height=15)
    carrito_tree.heading("ID", text="ID")
    carrito_tree.heading("Producto", text="Producto")
    carrito_tree.heading("Cantidad", text="Cantidad")
    carrito_tree.heading("Precio", text="Precio")
    ...
    carrito_tree.column("Precio", width=100)
    ...
    menu.add_command(label="Cambiar cantidad", command=lambda: modificar_cantidad(carrito, carrito_tree, total_label, productos))
